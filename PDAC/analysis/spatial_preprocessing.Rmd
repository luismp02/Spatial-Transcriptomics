...
params:
  ...
  basal_thr: 0.25
  classical_thr: 0.25
  sample_name: 'Visium_FFPE_V44L23-391_A'
...
#uscore_final_table <- NA
#uscore_final_table <- read.csv('all_UScores', row.names = 1)
```
...
```{r, eval=TRUE}
plot(density(unlist(ucell_obj@meta.data$classical_UCell)), lwd = 2)
```

```{r, eval=TRUE}
plot(density(unlist(ucell_obj@meta.data$basal_UCell)), lwd = 2)
```


```{r, eval=FALSE}
Uscore_table <- ucell_obj@meta.data[,c('classical_UCell', 'basal_UCell')]
Uscore_table$data <- sample_name
rownames(Uscore_table) <- paste0(rownames(Uscore_table),'_',sample_name)
dim(uscore_final_table)
uscore_final_table <- rbind(uscore_final_table, Uscore_table)
dim(uscore_final_table)
write.csv(uscore_final_table, file = 'all_UScores', row.names = TRUE)
```

```{r, eval=FALSE}
plot(density(unlist(uscore_final_table$classical_UCell)), lwd = 2)
```

```{r, eval=FALSE}
plot(density(unlist(uscore_final_table$basal_UCell)), lwd = 2)
```
